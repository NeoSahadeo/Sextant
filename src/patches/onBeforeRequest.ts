...
export default function on_before_request() {
	session.defaultSession.webRequest.onBeforeRequest((details, callback) => {
		const should_skip: boolean = block_domain.block_domain(details.url);
		if (should_skip) return callback({ cancel: true });

		const redirect = request_limit.request_limit(details.url);
		if (redirect) {
